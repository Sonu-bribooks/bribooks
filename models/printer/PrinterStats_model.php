...
class PrinterStats_model extends CI_Model {
	public function printerAssignData($data = []) {
		$this->db->select("
			...
			GROUP_CONCAT(distinct printer_assignment.code) as assignment_code,
		");

		if (!empty($data['option_type'])) {
			$this->db->where_in('printer_assignment.option_type', $data['option_type']);
		}

		if (isset($data['assign_printer_id'])) {
			$this->db->where('printer_assignment.printer_id', $data['assign_printer_id']);
		}
		...
		if (isset($data['startdate']) || isset($data['enddate'])) {
			$this->db->where('printer_assignment.date_added BETWEEN "' . date('Y-m-d H:i:s', strtotime($data['startdate'])) . '" and "' . date('Y-m-d 23:59:59', strtotime($data['enddate'])) . '"');
		}
		...
		$this->db->where('order.pickup_location_id', (int)$this->config->item('default_pickup_location_id'));
		$this->db->where('product._deleted', 0);
		$this->db->from('order');
		$this->db->join('printer_assign_logs as product', 'order.id=product.order_id', 'left');
		$this->db->join('book', 'product.product_id = book.id', 'left');
		$this->db->join('printer_assignment', 'printer_assignment.id = product.assignment_id', 'left');
	...
	public function printerBookStatus($data = []) {
		$this->db->select('
			printer_assign_logs.order_id as order_id,
			printer_assign_logs.product_id,
			book.name,
			printer_assign_logs.option,
			book.book_id,
			book.author_name,
			printer_assign_logs.quantity,
			printer_assign_logs.version,
			printer_assign_logs.date_added
		');

		if (isset($data['assign_printer_id'])) {
			$this->db->where('printer_assign_logs.printer_id', (int)$data['assign_printer_id']);
		}

		$this->db->where('printer_assign_logs.status', (int)$data['status']);
		$this->db->where('printer_assign_logs.version', (int)$data['version']);
		$this->db->where('printer_assign_logs.product_id', (int)$data['book_id']);
		$this->db->like('printer_assign_logs.option', $data['type']);
		$this->db->from('printer_assign_logs');
		$this->db->join('book_version as book', 'book.book_id = printer_assign_logs.product_id AND book.version = printer_assign_logs.version', 'left');
	...
	public function printerStats($data = []) {
		$this->db->select_sum('printer_assign_logs.quantity');
		...
		if (isset($data['option'])) {
			$this->db->like('printer_assign_logs.option', $data['option']);
		}
		...
		if (isset($data['date_added'])) {
			$this->db->where('DATE(printer_assign_logs.date_added)', date('Y-m-d', strtotime($data['date_added'])));
		}
	...
	public function todayData($data = []) {
		...
		if (isset($data['assign_printer_id'])) {
			$this->db->where('printer_assign_logs.printer_id', $data['assign_printer_id']);
		}
		...
		$this->db->where('DATE(printer_assign_logs.date_added)', date('Y-m-d'));
	...
	public function printerAssignDataSortByBalanced($data = []) {
		...
		if (!empty($data['search'])) {
			...
			$this->db->or_like('replace(book.isbn, "-", "")', $data['search'], 'after');
			$this->db->group_end();
		}
