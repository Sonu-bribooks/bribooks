...
class Page_model extends CI_Model {
	...
	public function resetSortOrder($book_id = 0) {
		$this->db->update('page', [
			'sort_order'	=> 0,
		], [
			'book_id'		=> (int)$book_id,
		...
		foreach ($pages as $sort_order => $page) {
			$this->db->update('page', [
	...
	public function checkSpamWords($words = '') {
		$escaped_words = $this->db->escape_like_str($words);

		return $this->db->select('word')
			->from('spam_words')
			->where('_deleted', 0)
			->where("REGEXP_LIKE('$escaped_words', CONCAT('\\\b', word, '\\\b'))", NULL, FALSE)
			->get()->result_array();
	}
}
