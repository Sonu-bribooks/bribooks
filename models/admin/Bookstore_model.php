...
class Bookstore_model extends CI_Model {
	...
	public function get_all($data = []) {
		$this->db->select('
			book_id as id, user_id, name, version, bookstore.slug, author_name, author_image, cover_image, category_id, category, bookstore.location, sold, views, bookstore.status, bookstore.date_added, bookstore.date_modified, bookstore.date_published
		');
		...
		if (isset($data['quantity'])) {
			$this->db->where('sold', (int)$data['quantity']);
		}

		if (!empty($data['quantity_le'])) {
			$this->db->where('sold <= ', (int)$data['quantity_le']);
		}

		if (!empty($data['quantity_ge'])) {
			$this->db->where('sold >= ', (int)$data['quantity_ge']);
		}
		...
		if (isset($data['event_id'])) {
			$this->db->where(sprintf('bookstore.book_id in (select book_id from event_book where event_id = %s)', (int)$data['event_id']));
		}

		if (isset($data['site_id']) || isset($data['email'])) {
			if (isset($data['site_id'])) {
				$this->db->where('users.site_id', (int)$data['site_id']);
			}

			if (isset($data['email'])) {
				$this->db->where('users.email', trim($data['email']));
			}
