...
class EventBook_model extends CI_Model {
	...
	public function get_all($data = []) {
		...
		$this->db->select('
			event_book.*, book.user_id, 
			book.name as book_name, 
			book.date_published, 
			event.name as event_name, 
			event.end_date as event_end_date, 
			event.country_code,
			event.buying_options,
		');
		...
		if (isset($data['active_book'])) {
			$this->db->where('book_version.archived', 0);
		...
		if (isset($data['date_published'])) {
			$this->db->where('book_version.date_published <', $data['date_published']);
		}
		...
		if (isset($data['active_book']) || isset($data['date_published'])) {
			$this->db->join('book_version', 'book_version.book_id = event_book.book_id AND book_version.version = 1', 'left');
		}

		if (isset($data['site_id']) || isset($data['user_id']) || isset($data['grade']) || isset($data['section'])) {
			$this->db->join('users', 'users.id = book.user_id', 'left');
		}

		if (!empty($data['is_active_event'])) {
			$this->db->where(
				vsprintf('event_book.event_id IN (SELECT id FROM event where start_date <= \'%s\' AND end_date >= \'%s\')', [
					date('Y-m-d H:i:s'),
					date('Y-m-d H:i:s'),
				])
			);
