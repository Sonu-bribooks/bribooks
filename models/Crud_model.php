...
class Crud_model extends CI_Model {
	...
	public function get($site_id = 0) {
		...
		$this->db->where('settings.site_id', (int)$site_id);
	...
	public function get_all($data = []) {
		...
		if (isset($data['site_id'])) {
			$this->db->where('settings.site_id', (int)$data['site_id']);
		}
	...
	public function add_category() {
		...
		if ($this->input->post('site')) {
			...
			$this->site_model->addSitesByTable('category', [
				'column'	=> 'category_id',
	...
	{
		return $this->db->get_where('category', array('parent_id' => $parent_id))->result_array();
	}
	...
	public function update_setting_template($key, $value)
	{
		if ($this->db->get_where('settings', ['key' => $key])->row_array()) {
			$data['value'] = $value;
	...
	public function update_system_currency()
	{
		...
		$data['value'] = html_escape($this->input->post('exchange_rate'));
		$this->db->where('key', 'exchange_rate');
		$this->db->update('settings', $data);
	}
	...
	public function add_course($param1 = '')
	{
		...
		if ($this->input->post('site')) {
			...
			$this->site_model->addSitesByTable('course', [
				'column'	=> 'course_id',
				'id'		=> $course_id,
				'sites'		=> $this->input->post('site')
			]);
		...
		if ($data['status'] == 'approved') {
			$this->session->set_flashdata('flash_message', _l('course_added_successfully'));
		} elseif ($data['status'] == 'pending') {
			$this->session->set_flashdata('flash_message', _l('course_added_successfully') . '. ' . _l('please_wait_untill_Admin_approves_it'));
		} elseif ($data['status'] == 'draft') {
			$this->session->set_flashdata('flash_message', _l('your_course_has_been_added_to_draft'));
		}
	}
	...
	public function update_course($course_id, $type = '')
	{
		...
		if ($data['status'] == 'approved') {
			$this->session->set_flashdata('flash_message', _l('course_updated_successfully'));
		} elseif ($data['status'] == 'pending') {
			$this->session->set_flashdata('flash_message', _l('course_updated_successfully') . '. ' . _l('please_wait_untill_Admin_approves_it'));
		} elseif ($data['status'] == 'draft') {
			$this->session->set_flashdata('flash_message', _l('your_course_has_been_added_to_draft'));
		}
	}
		...
		if ($lesson_type == 'video') {
			...
			if ($lesson_provider == 'youtube' || $lesson_provider == 'vimeo') {
				if ($this->input->post('video_url') == '' || $this->input->post('duration') == '') {
					$this->session->set_flashdata('error_message', _l('invalid_lesson_url_and_duration'));
				...
				$this->session->set_flashdata('error_message', _l('invalid_lesson_provider'));
				redirect(site_url(strtolower($this->session->userdata('role')) . '/course_form/course_edit/' . $data['course_id']), 'refresh');
			}
		} else {
			if ($_FILES['attachment']['name'] == '') {
				$this->session->set_flashdata('error_message', _l('invalid_attachment'));
				redirect(site_url(strtolower($this->session->userdata('role')) . '/course_form/course_edit/' . $data['course_id']), 'refresh');
			} else {
	...
	public function enrol_student($user_id, $course_id = false, $mode = false)
	{
		if ($course_id) {
	...
	public function enrol_a_student_manually()
	{
		...
		if ($this->db->get_where('enrol', $data)->num_rows() > 0) {
			$this->session->set_flashdata('error_message', _l('student_has_already_been_enrolled_to_this_course'));
		} else {
			$data['date_added'] = date('Y-m-d H:i:s');
			$this->db->insert('enrol', $data);
			$this->session->set_flashdata('flash_message', _l('student_has_been_enrolled_to_that_course'));
		}
	}
	...
	public function enrol_to_free_course($course_id = '', $user_id = '')
	{
		...
		if ($course_details['is_free_course'] == 1) {
			...
			if ($this->db->get_where('enrol', $data)->num_rows() > 0) {
				...
				$data['date_added'] = date('Y-m-d H:i:s');
				$this->db->insert('enrol', $data);
				$this->session->set_flashdata('flash_message', _l('successfully_enrolled'));
			}
		} else {
			$this->session->set_flashdata('error_message', _l('this_course_is_not_free_at_all'));
	...
	public function course_purchase_from_enrol($user_id, $course_id, $amount_paid)
	{
		$data['user_id'] = $user_id;
		$data['payment_type'] = 'ReazorPay';
		$data['course_id'] = $course_id;
	...
	public function get_total_duration_of_lesson_by_course_id($course_id)
	{
		...
		return gmdate('H:i:s', $total_duration) . ' ' . _l('hours');
	}
	...
	public function sum_school_register($new = false, $site_code = '') {
	}

	public function sum_school_students($new = false, $site_code = '') {
	}

	public function sum_school_books($new = false, $site_code = '') {
	}

	public function sum_school_books_published($new = false, $site_code = '') {
	}

	public function sum_nyaf_ordered_book($new = false, $site_code = '') {
	}

	public function sum_nyaf_orderes($new = false, $site_code = '') {
	}

	// Event base count
	public function event_sum_all_school_register($new = false, $event_info = []) {
	}

	public function event_sum_all_school_students($new = false, $event_info = []) {
	}

	public function event_sum_school_students($new = false, $site_code = '', $event_info = []) {
	}

	public function event_sum_old_school_students($new = false, $site_code = '', $event_info = []) {
	}

	public function event_sum_school_books($new = false, $event_info = []) {

	}

	public function event_sum_school_books_published($new = false, $event_info = []) {
	}

	public function event_sum_nyaf_ordered_book($new = false, $event_info = []) {
	}

	public function event_sum_nyaf_orderes($new = false, $event_info = []) {
	}
}
