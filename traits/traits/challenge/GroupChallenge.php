...
trait GroupChallenge {
	public function getGroupChallenge() {
		$this->form_validation->set_rules('slug', _l('slug'), [
			'trim',
			'required',
			'min_length[10]',
			'max_length[255]',
			['slug', [$this->validate_model, 'event_challenge_group_slug']]
		]);

		self::_runFormValidation();

		if (!$this->json) {
			$challenge_info = $this->event_challenge_group_model->getBySlug($this->input->post('slug'));
	...
	public function getGroupChallenges() {
		...
		if (!$this->json) {
			...
			$results = $this->event_challenge_group_model->get_all([
	...
	public function getGroupChallengeRanks() {
		$this->form_validation->set_rules('event_id', _l('event'), [
			'trim',
			'required',
			'numeric',
		...
		$this->form_validation->set_rules('event_challenge_group_id', _l('event_challenge_group'), [
			'trim',
			'required',
			'numeric',
			['event_challenge_group', [$this->validate_model, 'event_challenge_group']]
		]);

		$this->form_validation->set_rules('group_id', _l('group'), [
			'trim',
			'required',
			'numeric',
			['event_league_group', [$this->validate_model, 'event_league_group']]
		]);
		...
		if (!$this->json) {
			...
			$event_challenge_group_info = $this->event_challenge_group_model->get($this->input->post('event_challenge_group_id'));

			$rank_results = $this->ranking_lib->getGroupRanks(
				$this->input->post('event_id'),
				$this->input->post('event_challenge_group_id'),
			...
			if (!empty($event_challenge_group_info) && !empty($event_challenge_group_info['min_published'])) {
				$rank_data 		= array_values($rank_results['ranks']);
				$min_publishd 	= $event_challenge_group_info['min_published'] ?? 0;
			...
			if ($this->session->userdata('user_id') || $this->input->post('user_id')) {
				...
				if (empty($this->json['user_rank'] = $this->ranking_lib->getUserGroupRank(
					$this->input->post('event_id'),
					$this->input->post('event_challenge_group_id'),
					$this->input->post('group_id'),
					$user_id,
					...
					$this->json['user_rank'] = $this->ranking_lib->getUserNoGroupRank(
						$this->input->post('event_id'),
						$this->input->post('event_challenge_group_id'),
						$this->input->post('group_id'),
						$user_id,
					...
					if (!empty(($event_challenge_group_info['min_published'] ?? 0))) {
						if (($this->json['user_rank']['score'] ?? 0) < ($event_challenge_group_info['min_published'] ?? 0)) {
	...
	public function getGroupChallengeUpdate($event_id = 0, $event_challenge_group_id = 0, $group_id = 0) {
		if (!$this->json) {
			$this->load->library('Ranking_lib', 'ranking_lib');
			$this->ranking_lib->getGroupUpdate($event_id, $event_challenge_group_id, $group_id, get_bb_user_id());
		}
	}
}
