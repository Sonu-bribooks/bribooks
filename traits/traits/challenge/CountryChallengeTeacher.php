...
trait CountryChallengeTeacher {
	public function getCountryChallengeTeacherRanks() {
		...
		if (!$this->json) {
			...
			$rank_results = $this->teacherranking_lib->getCountryRanks(
			...
			if ($this->input->post('user_id')) {
				$this->json['user_rank'] = $this->teacherranking_lib->getTeacherCountryRank(
					$this->input->post('event_id'),
					$this->input->post('event_challenge_country_id'),
					$this->input->post('country_id'),
	...
	public function getCountryChallengeTeacherUpdate($event_id = 0, $event_challenge_country_id = 0, $country_id = 0) {
		if (!$this->json) {
			$this->load->library('TeacherRanking_lib', 'teacherranking_lib');
			$this->teacherranking_lib->getCountryUpdate($event_id, $event_challenge_country_id, $country_id, get_bb_user_id());
		}
	}
}
