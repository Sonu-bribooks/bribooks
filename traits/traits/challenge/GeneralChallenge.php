...
trait GeneralChallenge {
	public function getGeneralChallenge() {
		$this->form_validation->set_rules('slug', _l('slug'), [
			'trim',
			'required',
			'min_length[10]',
			'max_length[255]',
			['slug', [$this->validate_model, 'event_challenge_general_slug']]
		]);

		self::_runFormValidation();

		if (!$this->json) {
			$challenge_info = $this->event_challenge_general_model->getBySlug($this->input->post('slug'));
			...
			$challenge_data = [
				...
				'min_book_sold'	=> $challenge_info['book_sold'],
				'max_book_sold'	=> $challenge_info['max_book_sold'],
	...
	public function getGeneralChallenges() {
		...
		if (!$this->json) {
			...
			$results = $this->event_challenge_general_model->get_all([
	...
	public function getGeneralChallengeRanks() {
		$this->form_validation->set_rules('event_id', _l('event'), [
			'trim',
			'required',
			'numeric',
		...
		$this->form_validation->set_rules('event_challenge_general_id', _l('event_challenge_general'), [
			'trim',
			'required',
			'numeric',
			['event_challenge_general', [$this->validate_model, 'event_challenge_general']]
		]);

		$this->form_validation->set_rules('general_id', _l('general'), [
			'trim',
			'required',
			'numeric',
		]);
		...
		if (!$this->json) {
			...
			$rank_results = $this->ranking_lib->getGeneralRanks(
				$this->input->post('event_id'),
				$this->input->post('event_challenge_general_id'),
				$this->input->post('general_id'),
			...
			if ($this->session->userdata('user_id') || $this->input->post('user_id')) {
				...
				if (empty($this->json['user_rank'] = $this->ranking_lib->getUserGeneralRank(
					$this->input->post('event_id'),
					$this->input->post('event_challenge_general_id'),
					$this->input->post('general_id'),
					$user_id,
					...
					$this->json['user_rank'] = $this->ranking_lib->getUserNoGeneralRank(
						$this->input->post('event_id'),
						$this->input->post('event_challenge_general_id'),
						$this->input->post('general_id'),
						$user_id,
			...
			$this->json['heading'] = 'Barnes List';
			$this->json['state'] = $state_info['name'] ?? '';
		}
	}

	public function getGeneralChallengeUpdate($event_id = 0, $event_challenge_general_id = 0, $general_id = 0) {
		if (!$this->json) {
			$this->load->library('Ranking_lib', 'ranking_lib');
			$this->ranking_lib->getGeneralUpdate($event_id, $event_challenge_general_id, $general_id, get_bb_user_id());
		}
	}
}
