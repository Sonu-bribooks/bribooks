...
trait SchoolTeacherRanking {
	public function updateSchoolRank($data = []) {
		...
		$challenge_info = $this->event_challenge_school_model->get_all([
			'type'					=> 'teacher',
		...
		if ($rank_info = $this->ranking_school_model->get_all([
			'event_challenge_school_id'	=> (int)$challenge_info['id'],
			'event_id'					=> (int)$event_info['id'],
			'teacher_id'				=> (int)$teacher_info['id'],
		])['rows'][0] ?? []) {
			$rank_id = $rank_info['id'];

			$this->ranking_school_model->edit($rank_info['id'], [
				'score'					=> $no_published,
		...
		self::_pushUpdate($rank_id, $no_published, 'school');
	}

	public function getSchoolRanks($event_id = 0, $event_challenge_school_id = 0, $school_id = 0, $page = 1, $search = NULL) {
		$ranks 		= [];
		$rank_key 	= self::_getKey(compact('event_id', 'event_challenge_school_id', 'school_id'), 'school');
	...
	public function getTeacherSchoolRank($event_id = 0, $event_challenge_school_id = 0, $school_id = 0, $user_id = 0) {
		$rank_key 	= self::_getKey(compact('event_id', 'event_challenge_school_id', 'school_id'), 'school');
		...
		$user_rank = !empty($user_rank)
			? self::_formatSchoolRank($rank , $user_rank)
			: self::_genSchoolRank(compact('event_id', 'event_challenge_school_id', 'school_id', 'user_id'), 'school')
		;

		return $user_rank;
	}

	private function _genSchoolRank($rank_info = [], $type = 'school') {
		...
		return self::_formatSchoolRank(0, [
			'id'						=> 0,
			'rank'						=> 0,
			'school_id'					=> $school_info['school_id'],
			'event_id'					=> $rank_info['event_id'],
			'event_challenge_school_id'	=> $rank_info['event_challenge_school_id'],
			...
			'registered'	=> self::_getRegisteredCount([
				'event_id'	=> $rank_info['id'],
				'site_id'	=> $teacher_info['site_id'],
				'grade'		=> $teacher_info['grade'],
				'section'	=> $teacher_info['section'],
	...
	public function getSchoolUpdate($event_id = 0, $event_challenge_school_id = 0, $school_id = 0, $user_id = 0) {
		self::_updateLiveUser(
			compact('event_id', 'event_challenge_school_id', 'school_id', 'user_id'),
			$user_id,
			'school'
		);
	...
	private function _formatSchoolRank($rank = 0, $item = []) {
		return [
			...
			'event_challenge_school_id'	=> $item['event_challenge_school_id'],
			...
			'registered'				=> self::_getRegisteredCount([
				'event_id'	=> $item['event_id'],
				'site_id'	=> $item['school_id'],
				'grade'		=> $item['grade'],
				'section'	=> $item['section'],
			]),
			'message' 					=> self::_getSchoolMessage(array_merge($item, [
				'rank'					=> $rank,
			])),
		];
	}

	private function _getSchoolMessage($rank_info = []) {
		...
		$challenge_info = $this->event_challenge_school_model->get($rank_info['event_challenge_school_id']);

		if (!empty($challenge_info) && date('Y-m-d H:i:s') > $challenge_info['end_date']) {
			return sprintf(_li('%s is closed now!'), $challenge_info['name']);
		}

		return method_exists($this, sprintf('_getSchoolEventMessage_%s', $rank_info['event_id']))
			? self::{sprintf('_getSchoolEventMessage_%s', $rank_info['event_id'])}($no_published, $rank_info, $challenge_info)
			: self::_getSchoolEventMessage($no_published, $rank_info, $challenge_info);
	}

	private function _getSchoolEventMessage($no_published = 0, $rank_info = [], $challenge_info = []) {
		...
		if (!empty($challenge_info['max_published']) && $no_published < $challenge_info['max_published'] && !empty($rank_info['rank'])) {
			return vsprintf(_li('You need at least %s published %s to be school-level winner.'), [
		...
		if (!empty($rank_info['rank']) && $rank_info['rank'] > 3) {
			$top_score 		= self::_getRankScore(3, $rank_info, false, 'school');
		...
		if (empty($rank_info['rank'])) {
			return vsprintf(_li('Your school has not yet reached the School League. You need at least %s published %s to qualify for school-level rankings.'), [
