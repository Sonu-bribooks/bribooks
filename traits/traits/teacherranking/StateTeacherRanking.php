...
trait StateTeacherRanking {
	...
	public function getTeacherStateRank($event_id = 0, $event_challenge_state_id = 0, $state_id = 0, $user_id = 0) {
		$rank_key 	= self::_getKey(compact('event_id', 'event_challenge_state_id', 'state_id'), 'state');
	...
	private function _getStateMessage($rank_info = []) {
		...
		$challenge_info = $this->event_challenge_state_model->get($rank_info['event_challenge_state_id']);

		if (!empty($challenge_info) && date('Y-m-d H:i:s') > $challenge_info['end_date']) {
			return sprintf(_li('%s is closed now!'), $challenge_info['name']);
		}

		return method_exists($this, sprintf('_getStateEventMessage_%s', $rank_info['event_id']))
			? self::{sprintf('_getStateEventMessage_%s', $rank_info['event_id'])}($no_published, $rank_info, $challenge_info)
			: self::_getStateEventMessage($no_published, $rank_info, $challenge_info);
	}

	private function _getStateEventMessage($no_published = 0, $rank_info = [], $challenge_info = []) {
		...
		if (!empty($challenge_info['max_published']) && $no_published < $challenge_info['max_published'] && !empty($rank_info['rank'])) {
			return vsprintf(_li('You need at least %s published %s to be state-level winner.'), [
		...
		if (!empty($rank_info['rank']) && $rank_info['rank'] > 3) {
			$top_score 		= self::_getRankScore(3, $rank_info, false, 'state');
		...
		if (empty($rank_info['rank'])) {
			return vsprintf(_li('Your school has not yet reached the State League. You need at least %s published %s to qualify for state-level rankings.'), [
