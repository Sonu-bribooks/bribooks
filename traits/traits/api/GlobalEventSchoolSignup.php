...
trait GlobalEventSchoolSignup {
	public function globalEventSendSchoolOtp() {
	...
	public function globalEventVerifySchoolOtp() {
		...
		if (!$this->json) {
			$lead_info = $this->school_lead_model->get($this->input->post('lid'));

			if (!empty($valid_school = $this->lead_verification_code_model->get_all([
				...
				if (
					...
					!empty($school_info['site_id']) &&
					!empty($this->event_site_model->getEventIdBySiteId($lead_info['event_id'], $school_info['site_id']))
				) {
					$this->json['success'] 	= _l( 'Your_school_is_already_registered');
					return;
				}
	...
	private function _updateGlobalSchoolUser($lead_info = []) {
		...
		if (empty($school_user_info)) {
			$school_user_id = $this->school_user_model->add([
				...
				'mobile_verified' 	=> $lead_info['type'] == 'mobile' ? 1 : 0,
				'email_verified' 	=> $lead_info['type'] == 'email' ? 1 : 0,
				'ip'				=> $lead_info['ip'],
			...
			$update_data = [
				...
				'email' 			=> $lead_info['email'],
				'mobile' 			=> $lead_info['mobile'],
				'mobile_verified' 	=> $lead_info['type'] == 'mobile' ? 1 : $school_user_info['mobile_verified'],
				'email_verified' 	=> $lead_info['type'] == 'email' ? 1 : $school_user_info['email_verified'],
				'ip'				=> $lead_info['ip'],
				'timezone'			=> $lead_info['timezone'],
	...
	private function _updateGlobalSiteDetails($lead_info) {
		$update_site_data = [
	...
	private function _updateGlobalSchoolDetails($lead_info) {
		$update_school_data = [
	...
	private function _addGlobalSiteDetails($lead_info = []) {
		$site_info = [];
	...
	private function _addGlobalSchoolDeatils($lead_info = []) {
		if (!empty($lead_info)) {
			$site_info = [];
	...
	private function _validateGlobalSchoolLead($data = []) {
