...
trait DropshipperAssignments {
	public function dropshipper_assignment($param1 = '', $param2 = '') {
		if ($this->session->userdata('dropShipper') != true || !in_array($this->session->userdata('role_id'), [_dropshipper_role()])) {
			redirect(base_url('login'), 'refresh');
		}

		$data['page_name'] 			= 'dropshipper_assignment/index';
		$data['page_title'] 		= _l('dropshipper_assignment');
		$data['action_ajax'] 		= base_url('dropShipper/ajax_dropshipper_assignment');
	...
	public function ajax_dropshipper_assignment() {
		$json['data'] = [];
		...
		$results = $this->dropshipper_assignment_model->get_all($filter_data);
		...
		foreach ($results['rows'] ?? [] as $key => $result) {
			...
			if (in_array($this->session->userdata('role_id'), [_dropshipper_role()])) {
				$total_count = $this->dropshipper_order_model->printerStats([
				...
				$qa_qc_lots_info = $this->dropshipper_order_model->getQaQcAssignCount([
					'assignment_id'		=> $result['id'],
				]);
			...
			$json['data'][] = [
				...
				'assignment_code'=> $result['code'] . vsprintf('<br /><a target="_blank" href="%s" title="View Book Titles">View Book Titles</a>', [base_url('dropShipper/book_titles/'.$result['code'])]),
				'stats'				=> $stats,
