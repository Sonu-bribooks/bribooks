...
trait CountrySchoolRanking {
	...
	public function getSchoolCountryRank($event_id = 0, $event_challenge_country_id = 0, $country_id = 0, $user_id = 0) {
		$rank_key 	= self::_getKey(compact('event_id', 'event_challenge_country_id', 'country_id'), 'country');
	...
	private function _genCountryRank($rank_info = [], $type = 'country') {
		...
		$city_info 		= $this->city_model->get($school_info['city_id']);
		$country_info 	= _get_country_by_code($school_info['country_code']);
	...
	private function _formatCountryRank($rank = 0, $item = []) {
		return [
			'id'						=> $item['id'] ?? 0,
			'rank'						=> $rank,
			'event_challenge_country_id'=> $item['event_challenge_country_id'] ?? 0,
	...
	private function _getCountryMessage($rank = []) {
		$no_published = !empty($rank['score'])
			? $rank['score']
			: self::_getPublishedCount($rank['event_id'], $rank['school_id']);
		;

		$challenge_info = $this->event_challenge_country_model->get($rank['event_challenge_country_id']);
		...
		return method_exists($this, sprintf('_getCountryEventMessage_%s', $rank['event_id']))
			? self::{sprintf('_getCountryEventMessage_%s', $rank['event_id'])}($no_published, $rank, $challenge_info)
			: self::_getCountryEventMessage($no_published, $rank, $challenge_info);
	}

	private function _getCountryEventMessage($no_published = 0, $rank = [], $challenge_info = []) {
