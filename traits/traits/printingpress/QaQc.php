...
trait QaQc {
	public function book_titles($assignment_code = '') {
		if (empty($assignment_code) || empty($assignment_info = $this->printer_assignment_model->getByCode($assignment_code))) {
			redirect(base_url('printingPress/printer_assignment'), 'refresh');
		}
		...
		$data['action_ajax'] 	= base_url('printingPress/ajax_book_titles/'.$assignment_code);
		...
		if(!empty($qa_qc_lots_info)) {
			...
			$data['action_csv'] 		= site_url('printingPress/download_qaqc_csv/'.$assignment_code);
			$data['action_csv_logs'] 	= site_url('printingPress/download_qaqc_logs_csv/'.$assignment_code);
			$data['action_complete'] 	= site_url('printingPress/qaqc_complete/'.$assignment_code);
		}
	...
	public function ajax_book_titles($assignment_code = '') {
		...
		if($assignment_code) {
			...
			foreach ($results['rows'] ?? [] as $key => $result) {
				...
				$balance_count = (int)$result['quantity']-(int)$accepted_count;
	...
	public function ajax_books_details_reset() {
		...
		if (!empty($qa_qc_lots_info = $this->printer_stats_model->getQaQcCount($filter_data))) {
			...
			$printer_extra_detail_info = $this->printer_extra_details_model->getByPrinterId($assignment_info['printer_id']);
			...
			if (($action != 2) && $quantity > 0) {
				...
				foreach ($order_ids as $order_id) {
					...
					foreach ($printer_assign_results as $printer_assign_info) {
						if(!in_array($printer_assign_info['status'], [3]))
							continue;
					}
	...
	public function qaqc_complete($assignment_code = '') {
		...
		if(empty($assignment_code) || empty($this->session->userdata('user_id'))) {
			...
			redirect(site_url('printingPress/printer_assignment'), 'refresh');
		}
		...
		if((int)$assignment_info['id']) {
			...
			redirect(base_url('printingPress/book_titles/'.$assignment_code), 'refresh');
		}
	}
	...
	public function qaqcBulkAction() {
		if ($this->session->userdata('printingPress') != true || !in_array($this->session->userdata('role_id'), [15])) {
			redirect(site_url('login'), 'refresh');
		}

		if (empty($assignment_id = $this->input->post('assignment_id')) || empty($printer_assign_logs_ids = $this->input->post('ids'))) {
			redirect(base_url('printingPress/printer_assignment'), 'refresh');
		}
		...
		$json = [];
		foreach ($printer_assign_logs_ids as $printer_assign_logs_id) {
			$order_assign_log_info = $this->printer_assign_log_model->get($printer_assign_logs_id);

			$book_id = $order_assign_log_info['product_id'] ?? '';

			$filter_data = [];
			$filter_data['assignment_id'] = $assignment_id;
			$filter_data['book_id'] = $book_id;
			$filter_data['assign_printer_id'] = $assignment_info['printer_id'];

			if (!empty($order_assign_log_info) && !empty($result = $this->printer_stats_model->printerAssignData($filter_data)['rows'][0] ?? [])) {
				$order_ids = explode(',', $result['order_ids']);
				foreach ($order_ids as $order_id) {
					$this->order_model->edit($order_id, [
						'printing_status' 	=> 1
				...
				$balance_count = $quantity-(int)$accepted_count;
				...
				if ($stock_info = $this->book_stock_model->get_all([
					...
					$this->book_stock_model->edit($stock_info['id'], [
						'quantity'	=> (int)($stock_info['quantity'] + (int)$balance_count),
					]);
