...
trait Orders {
	...
	public function orders($param1 = NULL, $param2 = NULL) {
		...
		$data['action_ajax'] 	= base_url('printingPress/ajax_in_print_orders/1');
	...
	public function new_order($param1 = NULL, $param2 = NULL) {
		...
		$bw_inprint_orders = $this->printer_stats_model->printerAssignData([
		...
		$data['page_name'] 		= 'order/orders';
		$data['headeing'] 		= _l('orders');
		$data['page_title'] 	= _l('new_orders');
		$data['action_ajax'] 	= base_url('printingPress/ajax_order/1');
		$data['action_event'] 	= base_url('printingPress/ajax_order_event/1');
		$data['status'] 		= 1;
	...
	public function in_print_order($param1 = NULL, $param2 = NULL) {
		...
		$data['headeing'] 		= _l('orders');
		$data['page_title'] 	= _l('new_orders');
		$data['action_ajax'] 	= base_url('printingPress/ajax_order/2');
		$data['action_event'] 	= base_url('printingPress/ajax_order_event/2');
		$data['status'] 		= 2;
	...
	public function send_in_print($ids = '') {
		...
		redirect(base_url('/printingPress/in_print_order'));
	}
	...
	public function send_bulk_inprint() {
		...
		if ($this->input->post('ids')) {
			...
			$json['redirect'] = base_url('/printingPress/in_print_order');
		} else {
	...
	public function partial_print($book_id = '', $type = '', $version = '') {
		return;
		$data['page_name'] 		= 'order/printing_status';
		$data['headeing'] 		= _l('orders');
		$data['page_title'] 	= _l('new_orders');
	...
	public function send_partial_in_print() {
		return;
		$json = [];
	...
	public function verify_print($param1 = NULL, $param2 = NULL) {
		...
		$data['page_name'] 		= 'order/orders';
		$data['headeing'] 		= _l('verify_print');
		$data['page_title'] 	= _l('verify_print');
		$data['action_ajax'] 	= base_url('printingPress/ajax_order/4');
		$data['action_event'] 	= base_url('printingPress/ajax_order_event/4');
		$data['status'] 		= 4;
		$data['last_download'] 	= $this->printer_zip_download_model->get_all([
			'status'			=> 1,
			'printer_id'		=> (int)$this->session->userdata('user_id'),
	...
	public function send_verify_print($ids = '') {
		...
		redirect(base_url('/printingPress/verify_print'));
	}
	...
	public function send_bulk_verify_print() {
		...
		if ($this->input->post('ids')) {
			...
			$json['redirect'] = base_url('/printingPress/verify_print');
		} else {
			$json['error'] = _l('unknown_error');
		}
	...
	public function printed_order($param1 = NULL, $param2 = NULL) {
		...
		$data['headeing'] 		= _l('orders');
		$data['page_title'] 	= _l('new_orders');
		$data['action_ajax'] 	= base_url('printingPress/ajax_order/3');
		$data['action_event'] 	= base_url('printingPress/ajax_order_event/3');
		$data['status'] 		= 3;
	...
	public function send_printed($ids = '') {
		...
		redirect(base_url('/printingPress/printed_order'));
	}
	...
	public function send_bulk_prited() {
		...
		if ($this->input->post('ids')) {
			...
			$json['redirect'] = base_url('/printingPress/printed_order');
		} else {
			$json['error'] = _l('unknown_error');
		}
	...
	public function afs($param1 = NULL, $param2 = NULL) {
		...
		$data['status'] 		= 21;
		$data['action_ajax'] 	= base_url('printingPress/ajax_order/21');
		$data['action_event'] 	= base_url('printingPress/ajax_order_event/21');
	...
	public function ready_to_ship($param1 = NULL, $param2 = NULL) {
		...
		$data['status'] 		= 21;
		$data['action_ajax'] 	= base_url('printingPress/ajax_order/9');
		$data['action_event'] 	= base_url('printingPress/ajax_order_event/9');
	...
	public function return_order($param1 = NULL, $param2 = NULL) {
		...
		$data['status'] 		= 21;
		$data['action_ajax'] 	= base_url('printingPress/ajax_order/15');
		$data['action_event'] 	= base_url('printingPress/ajax_order_event/15');
	...
	public function reprint_order($param1 = NULL, $param2 = NULL) {
		...
		$data['status'] 		= 21;
		$data['action_ajax'] 	= base_url('printingPress/ajax_order/10');
		$data['action_event'] 	= base_url('printingPress/ajax_order_event/10');
	...
	private function _getDownloadLink($book_id, $version, $mrp = 0) {
		return vsprintf('<a href="%s" class="btn btn-primary btn-sm p-1">%s</a>', [
			base_url(sprintf('printingPress/printBook/%s/%s/0/1/%d', $book_id, $version, $mrp)),
			_l('download_book')
		]);
	}
}
