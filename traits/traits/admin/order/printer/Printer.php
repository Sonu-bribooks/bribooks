...
trait Printer {
	public function test_printer_assign() {
		return;
		$results = $this->db->get_where('printer_assign_logs', [
			'assignment_id'	=> 30,
		])->result_array();
	...
	public function book_wise_new_orders() {
	...
	public function ajax_book_wise_new_orders() {
		$json['data'] = [];
		...
		$results = $this->printer_stats_model->bookWiseOrders($filter_data);
	...
	public function printer() {
		$this->load->helper('printer_stats');

		$cache_key = 'printer_stats';

		$data = json_decode($this->cache->get($cache_key), true);

		if (empty($data)) {
			$data['order_stats'] 				= _get_printer_order_stats();
			$data['order_stats_paperback'] 		= _get_printer_order_stats(1, 'Paperback');
			$data['order_stats_hardcover'] 		= _get_printer_order_stats(1, 'Hard Cover');
			$data['order_stats_blackwhite'] 	= _get_printer_order_stats(2, 'Black White');

			$data['reprint_stats'] 				= _get_printer_count_copies();
			$data['reprint_stats_paperback']	= _get_printer_count_copies('Paperback');
			$data['reprint_stats_hardcover']	= _get_printer_count_copies('Hard Cover');
			$data['reprint_stats_blackwhite']	= _get_printer_count_copies('Black White');

			$printers = $this->student_model->get_by_role_id_in([12, 15]);

			$data['printers'] = [];

			foreach ($printers as $key => $value) {
				$data['printers'][] = _get_printer_wise_stats($value);
			}

			$this->cache->save($cache_key, json_encode($data), 60);
		}

		$data['page_name']		= 'printer/index';
		$data['page_title']		= _l('printer');

		$this->load->view('backend/index', $data);
	}

	public function ajax_assign_order_to_printer($option_type = '1', $new_products = [], $self_call = false) {
		...
		if ($this->input->post('ids')) {
			...
			$assignment_id = $this->printer_assignment_model->get_all([
			...
			if (empty($assignment_id)) {
				$assignment_id = $this->printer_assignment_model->add([
					'printer_id'	=> (int)$this->input->post('printer_id'),
			...
			foreach ($order_ids as $order_id) {
				...
				if (in_array($order_info['status'], [21, 91, 92, 94]))
					continue;
				...
				if (empty($self_call) && in_array($order_info['status'], [93])) {
					...
					$this->stock_lib->subtract_stock($order_id);
				} else {
				...
				if ($this->printer_assign_log_model->get_all([
					'printer_id'	=> (int)$this->input->post('printer_id'),
					'order_id'		=> (int)$order_id,
				])['total'] == 0) {
			...
			$this->printer_assignment_model->edit($assignment_id, [
