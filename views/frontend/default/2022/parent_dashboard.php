	...
	<div class="container">
		...
		<div class="row">
			<div class="col-sm-4 mb-4 text-center">
				...
				<div
					class="gameList<?php echo strpos(mb_strtolower(trim($enrol_info['course'])), 'python') !== false ? 'Python' : 'Blockly'; ?>"
				></div>
				<?php } ?>
			</div>
		</div>
...
<?php if ($schedule) { ?>
<div id="timer-wrap">
	<p class="text-center alert alert-primary" id="zoom-note">
		<?php echo _li('Your Webinar will start in'); ?>
	</p>
		...
		<div class="row">
			<div class="col-sm-12">
				...
				<div>
					...
					Hey <?php echo $this->session->name; ?> ! I am Dev <span class="dev-icon"></span></h4><br /><br />
...
cool prizes, medals and certificates.</div>
			</div>
			<div class="col-sm-12">
				<div class="event-item">
					<a
						class="btn btn-success btn-kb btn-info"
						href="<?php echo base_url('home/webinar'); ?>"
					><?php _el('join_webinar'); ?></a>
				</div>
				...
				<div class="event-item">
					<a
						class="btn btn-success btn-kb btn-secondary disabled cert-button"
						href="#"
...
function enterGame(mode, gameId) {
	$.ajax({
		...
		success: function(json) {
			...
			if(json.code == 0) {
				<?php if (in_array($this->session->userdata('user_email'), TESTING_EMAILS)) { ?>
					window.location.href = "<?=GAME_URL_NEW?>";
				<?php } else { ?>
...
$('#zoom-counter').countdown({
	...
	callback: function(days, hours, minutes, second) {
		...
		// if (!loaded && minutes === 39 && second === 0) {
			$('#zoom-counter').hide();
			$('#zoom-note').html('<?php echo _li('Your webinar has been started..'); ?> <a id="zoom-url" href="<?php echo site_url('home/webinar'); ?>" class="btn btn-success"><?php _el('click_here_to_join'); ?></a>');
			loaded = true;
		}
	}
})
