...
foreach ($my_courses as &$my_course) {
	if (strtotime($my_course['renewal_date'] . ' +1 week') > time()) {
		$my_course['renewal'] = true;
		$my_course['payment_link'] = site_url('home/renewal/' . $this->enrol_model->generatePaymentLink($my_course['id']));
	}
...
<section class="page-header-area my-course-area">
	<div class="container">
		<div class="row">
			<div class="col">
				<h1 class="page-title"><?php echo _l('my_courses'); ?></h1>
				<br />
				<?php if (0) { ?>
				<ul>
				  <li class="active"><a href="<?php echo site_url('home/my_courses'); ?>"><?php echo _l('all_courses'); ?></a></li>
	...
	<div class="container">
		<div class="row align-items-baseline">
			<div class="col-lg-6">
				<div class="my-course-filter-bar filter-box">
					...
					<div class="btn-group">
						<a href="<?php echo site_url('home/my_courses'); ?>" class="btn reset-btn" disabled><?php echo _l('reset'); ?></a>
					</div>
				</div>
			</div>
			<div class="col-lg-6">
				<div class="my-course-search-bar">
					<form action="">
						<div class="input-group">
							<input type="text" class="form-control" placeholder="<?php echo _l('search_my_courses'); ?>" onkeyup="getCoursesBySearchString(this.value)">
		...
		<div class="row no-gutters" id = "my_courses_area">
			<?php foreach ($my_courses as $my_course):
				$course_details = $this->crud_model->get_course_by_id($my_course['course_id'])->row_array();
				$instructor_details = $this->user_model->get($course_details['user_id']);?>
					...
					<div class="course-box-wrap">
							<div class="course-box">
								...
								<div class="course-details">
									...
									<div class="rating your-rating-box" onclick="event.preventDefault();" data-toggle="modal" data-target="#EditRatingModal">
										...
										<p class="your-rating-text" id = "<?php echo $my_course['course_id']; ?>" onclick="getCourseDetailsForRatingModal(this.id)">
											<span class="your"><?php echo _l('your'); ?></span>
											<span class="edit"><?php echo _l('edit'); ?></span>
											<?php echo _l('rating'); ?>
										</p>
									</div>
								</div>
								<div class="row" style="padding: 5px;">
									<div class="col-md-6">
										<a href="<?php echo site_url('home/course/'.slugify($course_details['title']).'/'.$my_course['course_id']); ?>" class="btn"><?php echo _l('course_detail'); ?></a>
									</div>
									<div class="col-md-6">
										 <a href="<?php echo site_url('home/lesson/'.slugify($course_details['title']).'/'.$my_course['course_id']); ?>" class="btn"><?php echo _l('start_lesson'); ?></a>
									</div>
									<?php if (0) { ?>
									<div class="col-md-12 m-t-20 re_schedule">
										 <a href="<?php echo site_url('home/re_schedule/'.slugify($course_details['title']).'/'.$my_course['course_id']); ?>" class="btn"><?php echo _l('schedule'); ?></a>
									</div>
									<?php } ?>

									<?php if (!empty($my_course['renewal'])) { ?>
									<div class="col-md-12 m-t-20 re_schedule">
										 <a href="<?php echo $my_course['payment_link']; ?>" class="btn btn-danger"><?php echo _l('pay_fee'); ?></a>
									</div>
									<?php } ?>
								</div>
							</div>
