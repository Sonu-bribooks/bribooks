	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="<?php echo base_url('dropShipper/qaqc_action'); ?>" method="post" id="form-qaqc-order">
...
$(document).on('click', '.btn-qaqc', function() {
	...
	$.post('<?=base_url('dropShipper/ajax_books_details')?>', {book_id: $el.data('id'), assignment_id: $("#assignment_id").val(), version: $("#version").val(), option: $("#option").val()}, json => {
		if (json.products) {
...
$(document).on('click', '.btn-qaqc-reset', function() {
	...
	if (confirm('<?php echo _li('Are you sure to reset?'); ?>')) {
		$.post('<?=base_url('dropShipper/ajax_books_details_reset')?>', {book_id: $el.data('id'), assignment_id: $("#assignment_id").val(), version: $el.data('version'), option: $el.data('option')}, json => {
			if (json.success) {
...
$(document).on('click', '.btn-qaqc-reset-rejected-count', function() {
	...
	if (confirm('<?php echo _li('Are you sure to reset rejected count?'); ?>')) {
		$.post('<?=base_url('dropShipper/ajax_books_details_reset_rejected_count')?>', {book_id: $el.data('id'), assignment_id: $("#assignment_id").val(), version: $el.data('version'), option: $el.data('option')}, json => {
			if (json.success) {
				$('.rejected').css('display', 'none');
				$('#action').val('');
...
$(document).on('click', '.qaqc-complete', function(event) {
	if (confirm('Are you sure?')) {
		$.ajax({
			...
			url: '<?=base_url('dropShipper/qaqc_complete')?>',
...
$(document).on('click', '.bulk-qaqc-action', function(event) {
	...
	if (confirm('<?=_li('Are you sure? Total Book Titles: ')?>' + ids.length)) {
		$.ajax({
			url: '<?=base_url('dropShipper/qaqcBulkAction')?>',
			type: 'POST',
			data: {
				ids: ids,
