		...
		<div class="card">
			<div class="card-body">
				<h4 class="page-title"> <i class="mdi mdi-apple-keyboard-command title_icon"></i> <?php echo $page_title; ?>
					<a href = "<?php echo site_url('portal/enrol_student'); ?>" class="btn btn-outline-primary btn-rounded alignToTitle"><i class="mdi mdi-plus"></i><?php echo _l('enrol_student_offline'); ?></a>
				</h4>
			</div> <!-- end card body-->
		</div> <!-- end card -->
	</div><!-- end col-->
		...
		<div class="card">
			<div class="card-body">
				...
				<div class="row justify-content-md-center">
					<div class="col-xl-6">
						<form class="form-inline" action="<?php echo site_url('portal/enrolment/filter_by_date_range') ?>" method="get">
							<div class="col-xl-10">
				...
				<div class="table-responsive mt-4">
					<?php if (count($results) > 0): ?>
						<table id="basic-datatable" class="table table-striped table-centered mb-0">
							...
							<tbody>
								<?php foreach ($results as $key => $enrol): ?>
									<tr class="gradeU">
										...
										<td>
											<a href="<?php echo site_url('portal/user_form/edit_user_form/' . $enrol['user_id']); ?>" target="_blank"><b><?php echo $enrol['user']; ?></b> <i class="dripicons-pencil"></i><a><br>
										...
										<td><strong><a href="#" target="_blank"><?php echo ellipsis($enrol['course']); ?></a></strong></td>
											...
											<div class="dropright dropright">
												...
												<ul class="dropdown-menu">
													<li class="mt-1 text-center">
														<button type="button" data-toggle="modal" data-target="#payment-modal"class="btn btn-outline-info btn-icon btn-rounded btn-sm" onclick="$('input[name=enrol_id]').val(<?php echo $enrol['id']; ?>);$('input[name=amount]').val(<?php echo ($enrol_amount = $this->enrol_model->getRenewalAmount($enrol['id'])); ?>);"> <i class="dripicons-link"></i><?php _el('renewal_link'); ?> </button>
													</li>
													<li class="mt-1 text-center">
														<button type="button" data-toggle="modal" data-target="#edit-modal"class="btn btn-outline-success btn-icon btn-rounded btn-sm" onclick="$('input[name=enrol_id]').val(<?php echo $enrol['id']; ?>);getEnrol(<?php echo $enrol['id']; ?>);getEnrolEmis(<?php echo $enrol['id']; ?>, '<?php echo $enrol['emi_type']; ?>', <?php echo $enrol_amount; ?>);"> <i class="dripicons-pencil"></i> <?php _el('edit'); ?></button>
													</li>
													<li class="mt-1 text-center">
														<?php if ($enrol['archived']) { ?>
														<button type="button" class="btn btn-outline-success btn-icon btn-rounded btn-sm" onclick="confirm_modal('<?php echo site_url('portal/enrol_archive/' . $enrol['id']); ?>');"> <i class="dripicons-clockwise"></i> <?php $enrol['archived'] ? _el('restore') : _el('archive'); ?></button>
														<?php } else {  ?>
														<button type="button" class="btn btn-outline-danger btn-icon btn-rounded btn-sm" onclick="confirm_modal('<?php echo site_url('portal/enrol_archive/' . $enrol['id']); ?>');"> <i class="dripicons-archive"></i> <?php $enrol['archived'] ? _el('restore') : _el('archive'); ?></button>
														<?php } ?>
													</li>
												</ul>
											</div>
				...
				<div class="modal fade" id="edit-modal">
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-body p-3">
								<form action="<?php echo site_url('portal/updateEnrol'); ?>" method="post" id="form-enrol">
									<input type="hidden" name="enrol_id" value="" />
...
<script>
const getEnrolEmis = (enrol_id, emi_type, amount) => {
	let fd = new FormData();
	fd.append('enrol_id', enrol_id);

	$('#enrol_amount').val(amount);

	submitForm('<?php echo site_url('portal/getEnrolEmis'); ?>', fd, json => {
		if (json.emis) {
			let html = '';
			emis = json.emis;
...
const getEnrol = (enrol_id) => {
	...
	submitForm('<?php echo site_url('portal/getEnrol'); ?>', fd, json => {
		if (json.success) {
