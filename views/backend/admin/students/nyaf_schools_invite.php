	...
	<div class="col-xl-12">
		<div class="card">
			<div class="card-body">
				...
				<a href="<?php echo $action_csv; ?>" class="btn btn-outline-primary btn-rounded alignToTitle ml-2"><i class="mdi mdi-download"></i> <?php echo _li(' School CSV'); ?></a>
				<a href="<?php echo $action_zip; ?>" class="btn btn-outline-primary btn-rounded alignToTitle"><i class="mdi mdi-download"></i> <?php echo _li(' School PDFs'); ?></a>
			</div> <!-- end card body-->
		</div> <!-- end card -->
	</div><!-- end col-->
</div>
...
$(function() {
	$(document).on('click', '#btn-export', function(e) {
		...
		if (confirm('<?=_l('are_you_sure?')?>')) {
			...
			window.location = '<?= base_url('admin/export_school_invites/'); ?>/1?' + filters.join('&');
		}
	})
});
</script>