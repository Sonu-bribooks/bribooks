	...
	<div class="col-xl-12">
		<div class="card">
			<div class="card-body">
				...
				<a href="<?php echo $action_csv; ?>" class="btn btn-outline-primary btn-rounded alignToTitle ml-2"><i class="mdi mdi-download"></i> <?php echo _li(' Author CSV'); ?></a>
				<a href="<?php echo $action_zip; ?>" class="btn btn-outline-primary btn-rounded alignToTitle"><i class="mdi mdi-download"></i> <?php echo _li(' Author PDFs'); ?></a>
			</div> <!-- end card body-->
		</div> <!-- end card -->
	</div><!-- end col-->
</div>
			...
			<div class="card-body">
				<form class="form" action="#" method="post" id="form-filter">
					<div class="row">
						...
						<div class="col-sm-6">
							<div class="form-group mb-2">
								<label><?=_l('select_user_type')?></label>
								<select
									class="form-control input-filter select2"
									data-toggle="select2"
									name="user_type"
								>
									<option value="student"><?=_l('student')?></option>
									<option value="teacher"><?=_l('teacher')?></option>
								</select>
							</div>
						</div>
					</div>
...
$(function() {
	$(document).on('click', '#btn-export', function(e) {
		...
		if (confirm('<?=_l('are_you_sure?')?>')) {
			...
			window.location = '<?= base_url('admin/export_author_invites/'); ?>/1?' + filters.join('&');
		}
	})
});
</script>
