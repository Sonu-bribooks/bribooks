	...
	<div class="col-xl-12">
		<div class="card mb-2">
			<div class="card-body p-2">
				...
				<div class="alignToTitle">
					<?php if (!empty($assignment_code)) { ?>
					<button type="button" class="btn btn-primary rollbackBulk">
						<?=_l('Rollback_bulk')?>
					</button>
					<?php } ?>

					<button type="button" class="btn btn-primary ml-2" data-toggle="modal" data-target="#reprintBulkModal">
						<?=_l('Reprint_bulk')?>
					</button>

					<button type="button" class="btn btn-primary ml-2" data-toggle="modal" id="syncBulkBtn" data-target="#syncBulkModal">
						<?=_l('Sync_bulk')?>
					</button>

					<button type="button" class="btn btn-primary ml-2" data-toggle="modal" data-target="#printerAssignModal">
						<?=_l('Assign_printer')?>
					</button>
				</div>
...
<div class="modal fade" id="reprintModal" role="dialog" aria-labelledby="reprintModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="<?php echo base_url('admin/reprint_order/1'); ?>" method="post" id="form-reprint-order">
...
<!-- reprint bulk modal-->
<div class="modal fade" id="reprintBulkModal" role="dialog" aria-labelledby="reprintBulkModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="reprintBulkModalLabel"><?= _l('reprint_bulk_order') ?></h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<form action="javascript:;" method="post" id="form-reprint-bulk-order">
					<div class="form-group form-check">
						<input type="checkbox" class="form-check-input" name="use_different_printer_bulk" id="use_different_printer_bulk" />
						<label class="form-check-label" id="use_different_printer_bulk"><?=_l('use_different_printer_bulk')?></label>
					</div>
					<div class="form-group" id="use_different_printer_bulk_div" style="display: none;">
						<label><?php _el('select_printer'); ?></label>
						<select name="printer_id_bulk" class="form-control select2 printer" data-toggle="select2" id="reprint_bulk_printer_id">
							<option value=""><?=_l('select_printer')?></option>
							<?php foreach ($printer_list ?? [] as $key => $value) { ?>
							<option value="<?=$value['id']?>"><?=$value['first_name'] . ' ' . $value['last_name']?></option>
							<?php } ?>
					...
					<div class="form-group">
						<label for="comment"><?php _el('comment'); ?></label>
						<textarea name="comment_bulk" id="comment_bulk" rows="6" class="form-control" required></textarea>
					</div>
					<div class="form-group">
						<label for="order_history"><?php _el('add_to_order_history'); ?></label>
						<select name="order_history_bulk" id="order_history_bulk" class="form-control select2" data-toggle="select2">
							<option value="0"><?=_l('no')?></option>
							<option value="1"><?=_l('yes')?></option>
						</select>
					</div>
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary" data-dismiss="modal"><?=_l('close')?></button>
				<button type="submit" form="form-reprint-bulk-order" class="btn btn-primary"><?=_l('submit')?></button>
			</div>
		</div>
	</div>
</div>
			...
			<div class="card-body">
				<form class="form" action="#" method="post" id="form-filter">
					<div class="row">
							...
							<div class="form-group mb-2">
								...
								>
									...
									<?php foreach ($printer_assignment_list['rows'] ?? [] as $value) { ?>
										<option <?php if(!empty($assignment_code) && ($assignment_code == $value['code'])) { echo 'selected'; } ?> value="<?=$value['code']?>"><?=$value['code']?></option>
									<?php } ?>
								</select>
							</div>
						</div>
						...
						<!-- <div class="col-sm-3">
							<div class="form-group mb-2">
								<label><?=_l('has_multiple_items')?></label>
								<select
									class="form-control input-filter select2"
									data-toggle="select2"
									name="has_multiple_items"
								>
									<option value=""><?=_l('all')?></option>
									<option value="2"><?=_l('no')?></option>
									<option value="1"><?=_l('yes')?></option>
			...
			<div class="card-body">
				<div class="table-responsive">
					<ul class="nav nav-tabs" role="tablist">
						<li class="nav-item">
							<a class="nav-link <?= ($this->uri->segment(2)=='all_orders')?'active':'';?>" data-bs-toggle="tab" href="<?=base_url('admin/all_orders')?>" role="tab">
								<span class="d-block d-sm-none"><i class="fas fa-home"></i></span>
								<span class="d-none d-sm-block"><?=_l('all_order')?></span>
							</a>
						</li>
						<li class="nav-item">
							<a class="nav-link <?= ($this->uri->segment(2)=='orders')?'active':'';?>" data-bs-toggle="tab" href="<?=base_url('admin/orders')?>" role="tab">
...
$(function() {
	let columns_length = <?=in_array($this->session->userdata('role_id'), [1, 5, 18]) ? json_encode([10, 20, 50, 100, 200, 500, 1000]) : json_encode([10, 20, 50])?>;
...
$(function() {
	$(document).on('click', '#btn-export', function(e) {
		...
		if (confirm('<?=_l('are_you_sure?')?>')) {
			...
			window.location = '<?= base_url('admin/export_orders/0'); ?>?' + filters.join('&');
		}
	})
});
...
$('#form-reprint-bulk-order').on('submit', function(e) {
	e.preventDefault();
	e.stopPropagation();
	...
	if (($('#use_different_printer_bulk').prop('checked')) && ($('#reprint_bulk_printer_id').val() === '')) {
		error_notify('<?=_l('select_printer')?>')
		return false;
	}

	if (confirm('<?=_l('Are you sure?')?>')) {
		$.ajax({
			url: '<?=base_url('admin/reprint_bulk_order/1')?>',
			type: 'POST',
			data: {
				use_different_printer: $('#use_different_printer_bulk').prop('checked'),
				printer_id: $('#reprint_bulk_printer_id').val(),
				comment: $('#comment_bulk').val(),
				order_history: $('#order_history_bulk').val(),
				order_ids: ids
			},
			cache: false,
			success: function(json) {
				$('#reprintBulkModal').modal('hide');
				table.ajax.reload(null, false);
				json.success && success_notify(json.success)
				json.error && error_notify(json.error)
			}
...
$(document).on('click', '#syncBulkBtn', function(e) {
	e.preventDefault();
	e.stopPropagation();
	...
	if (confirm('<?=_l('Are you sure to sync?')?>')) {
		$.ajax({
			url: '<?=base_url('admin/sync_bulk_order')?>',
			type: 'POST',
			data: {
				order_ids: ids
			},
...
$(function() {
	$('#use_different_printer_bulk').on('change', function() {
		if ($(this).prop('checked')) {
			$('#use_different_printer_bulk_div').show();
		} else {
			$('#use_different_printer_bulk_div').hide();
		}
	});
	$('#use_different_printer_bulk').trigger('change');
});
...
$(document).on('click', '.btn-rollback', function(event) {
	if (confirm('Are you sure to rollback?')) {
		const fd = new FormData();
		fd.append('order_id', $(this).data('id'));
		fd.append('assignment_code', '<?= $assignment_code; ?>');
		submitForm('<?=base_url('admin/ajax_order_move_to_new')?>', fd, json => {
			if (json.success) {
				success_notify(json.success);
				table.ajax.reload(null, false);
			}
...
$('.rollbackBulk').on('click', function(event) {
	event.preventDefault();
	...
	let assignment_code = '<?= $assignment_code; ?>';

	if (confirm('<?=_li('Are you sure to rollback? Total Copies ')?>' + quantity)) {
		$.ajax({
			url: '<?=base_url('admin/ajax_order_move_to_new_bulk')?>',
			type: 'POST',
			data: {
				order_ids: ids,
