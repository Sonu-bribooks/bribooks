	...
	<div class="col-xl-12">
		<div class="card mb-2">
			<div class="card-body p-2">
				...
				<a href="<?= base_url('admin/downloadManifestPrint') ?>" class="btn btn-primary float-right mr-2" id="download_manifest">
					...
					<div class="col-sm-4 alignToTitle text-right">
						<div class="input-group">
							<select name="bulk-send" id="bulk-send" class="form-control bulk-send">
								...
								<option value="93" data-target="#escalateModal"><?=_l('escalate_order')?></option>
								<option value="93" data-target="#escalateRestoreModal"><?=_l('restore_order')?></option>
							</select>
...
<div class="modal fade" id="reprintModal" role="dialog" aria-labelledby="reprintModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="<?=$action_reprint ?>" method="post" id="form-reprint-order">
...
$(function() {
	<?php if (($this->uri->segment(2)) == 'ready_to_ship') { ?>
		$('#download_manifest').show();
	<?php } else { ?>
		$('#download_manifest').hide();
	<?php } ?>
	...
	table = $('#ajax-datatable').DataTable({
		...
		'createdRow': function(row, data, dataIndex) {
			...
			} else if (data.is_dropshipper) {
				$(row).css('background-color', '#33fff9');
			}
		}
	})
});
...
$('#bulk-action').on('click', function(event) {
	...
	if (status == 93) {
		const target = $('#bulk-send option:selected').data('target');
		$(target).find('input[name="order_id"]').val(ids.join(','));
		$(target).modal('show');
		return;
	}
...
$(function() {
	$(document).on('click', '#btn-export', function(e) {
		...
		if (confirm('<?=_l('are_you_sure?')?>')) {
			...
			window.location = '<?= base_url('admin/export_orders/' . ($navigation == 'ge_nav' ? 2 : 47)); ?>/1?' + filters.join('&');
		}
	})
});
