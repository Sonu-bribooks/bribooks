	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="<?php echo base_url('admin/qaqc_action'); ?>" method="post" id="form-qaqc-order">
...
<!-- qa qc modal-->
<div class="modal fade" id="qaqcRejectModal" role="dialog" aria-labelledby="qaqcRejectModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="qaqcRejectModalLabel">QA QC Reject</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<form action="<?php echo base_url('admin/qaqc_reject'); ?>" method="post" id="form-qaqc-reject-order">
					<input type="hidden" name="assignment_id" id="assignment_id" value="<?= $assignment_id ?? ''; ?>" />
					<input type="hidden" name="book_id" id="book_id" value="" />
					<input type="hidden" name="version" id="version" value="" />
					<input type="hidden" name="option" id="option" value="" />
					...
					<div class="form-group">
						<div id="qaqc-reject-form-content"></div>
					</div>
					<div class="form-group">
						<label><?php _el('quantity'); ?></label>
						<input type="text" name="quantity" id="reject_quantity" class="form-control" value="0" />
					</div>
					<div class="form-group">
						<label><?php _el('select_reason'); ?></label>
						<select name="reason" class="form-control" id="reject_reason">
							<option value=""><?=_l('select_reason')?></option>
							<option value="binding">Binding</option>
							<option value="content">Content</option>
							<option value="sku_mismatch">SKU Mismatch</option>
							<option value="torn_paper">Torn Paper</option>
					...
					<div class="form-group">
						<label for="comment"><?php _el('comment'); ?></label>
						<textarea name="comment" id="reject_comment" rows="4" class="form-control"></textarea>
					</div>
				</form>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary" data-dismiss="modal"><?=_l('close')?></button>
				<button type="submit" form="form-qaqc-reject-order" class="btn btn-primary btn-qaqc-reject-submit"><?=_l('submit')?></button>
			</div>
		</div>
	</div>
</div>
					...
					<table id="ajax-datatable" class="table table-striped table-centered mb-0">
						<thead>
							<tr>
								...
								<th><?php echo _l('manager_name'); ?></th>
...
$(document).on('click', '.btn-qaqc', function() {
	...
	$.post('<?=base_url('admin/ajax_books_details')?>', {book_id: $el.data('id'), assignment_id: $("#assignment_id").val(), version: $("#version").val(), option: $("#option").val()}, json => {
		if (json.products) {
...
$(document).on('click', '.btn-qaqc-reset', function() {
	...
	if (confirm('<?php echo _li('Are you sure to reset?'); ?>')) {
		$.post('<?=base_url('admin/ajax_books_details_reset')?>', {book_id: $el.data('id'), assignment_id: $("#assignment_id").val(), version: $el.data('version'), option: $el.data('option')}, json => {
			if (json.success) {
...
$(document).on('click', '.btn-qaqc-reset-rejected-count', function() {
	...
	if (confirm('<?php echo _li('Are you sure to reset rejected count?'); ?>')) {
		$.post('<?=base_url('admin/ajax_books_details_reset_rejected_count')?>', {book_id: $el.data('id'), assignment_id: $("#assignment_id").val(), version: $el.data('version'), option: $el.data('option')}, json => {
			if (json.success) {
				$('.rejected').css('display', 'none');
				$('#action').val('');
...
$(document).on('click', '.btn-qaqc-reject', function() {
	$el = $(this);
	...
	$.post('<?=base_url('admin/ajax_books_details')?>', {book_id: $el.data('id'), assignment_id: $("#assignment_id").val(), version: $("#version").val(), option: $("#option").val()}, json => {
		if (json.products) {
			const html = json.products.map((item, index) => '<label><?=_l('book_name')?>: ' + item.name + '</label><br /><label><?=_l('book_accepted')?>: ' + item.accepted_quantity + '</label><input type="hidden" name="accepted_quantity" id="accepted_quantity" value="' + item.accepted_quantity + '" readonly="" />');
			$('#qaqc-reject-form-content').html(html.join());
		}
	});
});
...
$(function() {
	...
	$('#form-qaqc-reject-order').on('submit', function(e) {
		// $('.btn-qaqc-reject-submit').prop('disabled', true);
		...
		if (isNaN($('#assignment_id').val()) || isNaN($('#book_id').val())) {
			error_notify('Please select valid details');
			$('.btn-qaqc-reject-submit').prop('disabled', false);
			return;
		}

		var accepted_quantity = parseInt($('#accepted_quantity').val());
		var quantity = parseInt($('#reject_quantity').val());
...
$(document).on('click', '.qaqc-complete', function(event) {
	if (confirm('Are you sure?')) {
		$.ajax({
			...
			url: '<?=base_url('admin/qaqc_complete')?>',
