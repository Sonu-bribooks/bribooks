			...
			<div class="col-sm-3 col-xs-6 mb-4">
				<div class="card shadow-none">
					<div class="card-body text-center">
						...
						<p class="text-muted font-15 mb-0"><b><?=$user['new']?></b> <?php echo _l('selected_date_total'); ?></p>
						<p class="text-muted font-15 mb-0"><b><?=$user['books_title']?></b> <?php echo _l('books_title_wise_total'); ?></p>
						<small class="text-success"><b><?=$user['total']?></b> <?php echo _l('current_month_total'); ?></small>
					</div>
				</div>
			</div>
			<?php } ?>
...
$(function() {
	$(document).on('click', '#btn-export', function(e) {
		...
		if (confirm('<?=_l('are_you_sure?')?>')) {
			...
			window.location = '<?=base_url('admin/ajax_export_logistic_qaqc_int'); ?>?' + filters.join('&');
		}
	});
	...
	$(document).on('submit', '#form-filter', function(e) {
		...
		window.location = '<?=base_url('admin/get_logistic_qaqc_int'); ?>?' + filters.join('&');
	})
});
