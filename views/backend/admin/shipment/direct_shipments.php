	...
	<div class="col-xl-12">
		<div class="card mb-2">
			<div class="card-body p-2">
				...
				<button type="button" class="btn btn-primary alignToTitle" id="bulk-cancel-action"><?=_l('bulk_cancel')?></button>
			</div>
		</div>
	</div>
</div>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="<?php echo base_url('admin/cancel_shipment_ds'); ?>" method="post" id="form-cancel-shipment">
			...
			<div class="modal-footer">
				...
				<button type="submit" form="form-cancel-shipment" class="btn btn-primary"><?=_l('ok')?></button>
			</div>
		</div>
	</div>
</div>
...
$('#bulk-cancel-action').on('click', function(event) {
	event.preventDefault();
	...
	if (ids.length == 0) {
		error_notify('<?=_l('select_atleast_one_checkbox')?>')
		return false;
	}

	if (confirm('<?=_l('Are you sure to cancel bulk shipments?')?>')) {
		$.ajax({
			url: '<?=base_url('admin/bulk_cancel_shipment_ds')?>',
			type: 'POST',
			data: {
				ids: ids
			},
...
$(function() {
	$(document).on('click', '#btn-export', function(e) {
		...
		if (confirm('<?=_l('are_you_sure?')?>')) {
			...
			window.location = '<?=base_url('admin/export_direct_shipments')?>?' + filters.join('&');
		}
	})
});
...
$('#form-cancel-shipment').on('submit', function(e) {
	e.preventDefault();
	e.stopPropagation();
	$el = $(this);
	if (confirm('<?php echo _li('Are you sure to cancel shipment?'); ?>')) {
