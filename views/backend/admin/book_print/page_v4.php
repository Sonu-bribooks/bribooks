...
<head>
	...
	<style>
		...
		.printlinecover {
			padding: <?= (0 * 16 * $multiplier) * 0.75 ?>pt <?= (2.188 * 16 * $multiplier) * 0.75 ?>pt;
			position: absolute;
			top: 50%;
			left: 0;
			right: 0;
...
<body>
	...
	<?php foreach ($pages as $key => $page) { ?>
		<?php
			...
			if (!empty($text) && !_is_html($text) && _contains_markdown($text)) {
				$text = _parse_md($text);
			}

			if (!empty($text_2) && !_is_html($text_2) && _contains_markdown($text_2)) {
				$text_2 = _parse_md($text_2);
			}
		?>
