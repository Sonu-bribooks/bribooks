...
<head>
	...
	<style>
		...
		.book-info {
			...
			padding-bottom: <?= (0.313 * 16 * $multiplier + $fc_bleed + $bleed_size) * 0.75 ?>pt;
			padding-right: <?= (0.625 * 16 * $multiplier + $fc_bleed + $bleed_size) * 0.75 ?>pt;
			font-family: Signika;
			font-weight: 300;
			position: absolute;
			bottom: 0;
	...
	right: 1.563pt; */
		...
		.backcover .version {
			...
			display: block;
			font-size: <?= 0.5 * 16 * 0.75 * $multiplier ?>pt;
			margin-right: calc(50% - <?=$book['isbn'] ? 60 : 80?>px);
			margin-top: 2pt;
		}
		.mark {
			position: absolute;
		...
		.ms1 {
			top: 0;
			left: <?=$page_width + $trim_size + $bleed_size?>pt;
		}
		.ms2 {
			bottom: 0pt;
			left: <?=$page_width + $trim_size + $bleed_size?>pt;
...
<body>
	<div class="book-cover">
			...
			<div class="page" style="background-color: <?= $book['back_color'] ?>;">
				<div class="backcover">
					<div class="title-info">
						<h3 class="text-light book-title" style="font-size: <?= (strlen($book['name']) > 15 ? 1.15 : 1.75) * 16 * $multiplier * 0.75 ?>pt; font-weight: 600;">
							<?= $this->emoji_lib->parseNormal($book['name'], '15x15') ?>
						</h3>
						<p class="text-light author-name" style="font-size: <?= (strlen($book['author_name']) > 10 ? 0.65 : 0.875) * 16 * $multiplier * 0.75 ?>pt;">
							Written by <?= $this->emoji_lib->parseNormal($book['author_name'], '10x10') ?>
						...
						<div class="author-info">
							<div class="author-img">
								...
								<p class="text-light" style="font-size: <?= (strlen($book['author_name']) > 10 ? 0.5 : 0.75) * 16 * $multiplier * 0.75 ?>pt;">
									<?= $this->emoji_lib->parseNormal($book['author_name'], '10x10') ?>
								</p>
							</div>
							<div class="text-light author-bio">
								<p><?= $this->emoji_lib->parseNormal($book['author_bio'], '10x10') ?></p>
							</div>
						</div>
					</div>
		...
		<!-- spine -->
		<div class="book-spine" style="background-color: <?= $book['back_color'] ?>;">
