...
<head>
	...
	<style>
		...
		.printlinecover {
			padding: <?= (0.5 * 16 * $multiplier) * 0.75 ?>pt <?= (2.188 * 16 * $multiplier) * 0.75 ?>pt;
			position: absolute;
			top: 40%;
			left: 0;
			right: 0;
		...
		.printlinecover .publisher-info {
			font-size: <?= .7 * 16 * 0.75 * $multiplier ?>pt;
			font-weight: 300;
	...
	<?php foreach ($pages as $key => $page) { ?>
		...
		<div class="book-page">
			<div class="page" style="position:relative;">
				...
				<div class="page-text" style="position: absolute;width: <?= ($design['p']['w'] + 20) * $multiplier * 0.75 ?>pt; left: <?= ($design['p']['x'] * $multiplier + $bleed) * 0.75 ?>pt; top: <?= ($design['p']['y'] * $multiplier + $bleed) * 0.75 ?>pt; font-family: <?= $page['font_family'] ?>; font-size: <?= $page['font_size'] * $multiplier * 0.75 ?>pt; line-height: <?= $page['font_size'] * $multiplier * 0.75 + 10 ?>pt; font-weight: <?= $page['font_weight'] ?>; color: <?= $page['font_color'] ?>;">
					<?php echo html_entity_decode($this->emoji_lib->parseNormal($text, '15x15')); ?>
				</div>
