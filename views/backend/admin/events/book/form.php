			...
			<div class="card-body">
				<div class="col-lg-12">
					...
					<form class="required-form" action="<?php echo $action; ?>" method="post" enctype="multipart/form-data">
						...
						<div class="form-group">
							<label for="parent">Select Book<span class="required">*</span></label>
							<select class="form-control select2" data-toggle="select2" name="book_id" id="book_id" required>
								<option value="<?php echo !empty($details['book_id']) ? ($details['book_id']) : ''; ?>"><?php echo !empty($details['book_id']) ? ($details['book_id']) : get_phrase('please_select'); ?></option>
							</select>
						</div>
...
$(function() {
	...
	function checkBookEvent (event_id, book_id) {
		$.ajax({
			url: '<?php echo site_url('admin/check_book_in_event'); ?>',
			type: 'POST',
			dataType: 'json',
			data: {
				event_id : event_id,
			...
			success: function(data){
				console.log(data);
				if(data.status == true){
					error_notify('This book is already added in this event.');
	...
	$(document).on('change', '#event_id', function(e) {
		...
		if ($(this).val() != '' && $('#book_id').val() != '' && $(this).val() != 0 && $('#book_id').val() != 0) {
			checkBookEvent($(this).val(), $('#event_id').val());
		}
	})
	...
	$(document).on('change', '#book_id', function(e) {
		...
		if ($('#event_id').val() != '' && $(this).val() != '' && $('#event_id').val() != 0 && $(this).val() != 0) {
			checkBookEvent($('#event_id').val(),$(this).val());
		}
	})
});
</script>
