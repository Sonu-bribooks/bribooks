		...
		<div id="accordion">
			<div class="card mb-0">
				...
				<div id="collapse-type" class="collapse show" aria-labelledby="type" data-parent="#accordion">
					<div class="card-body">
						<form action="<?=$action_type?>" method="post" enctype="multipart/form-data" id="form-type" class="form-horizontal">
								...
								<div class="row">
									...
									<div class="col-sm-10">
										<select class="form-control select2 type" data-toggle="select2" name="type">
											...
											<option value="<?=$type?>"><?=_l($type)?></option>
											<?php } ?>
										</select>
			...
			<div class="card mb-0">
				...
				<div id="collapse-file" class="collapse" aria-labelledby="file" data-parent="#accordion">
					<div class="card-body">
						<form action="<?=$action_file?>" method="post" enctype="multipart/form-data" id="form-import" class="form-horizontal">
							<div class="form-group">
								<div class="row">
			...
			<div class="card mb-0">
				...
				<div id="collapse-mapping" class="collapse" aria-labelledby="mapping" data-parent="#accordion">
					<div class="card-body">
						<form action="<?=$action_save?>" method="post" enctype="multipart/form-data" id="form-import-save" class="form-horizontal">
...
$('#button-download').on('click', function() {
	location = '<?=$action_download?>/' + $('select[name="type"]').val();
});
</script>
...
function mapSelectedFields() {
	$('.field-header').each(function() {
		...
		$el.find(`option[value=${value}]`).prop('selected', true).trigger('change');
	});
}
	...
	submitForm($el.attr('action'), new FormData($el[0]), json => {
		...
		if (json.headers && json.headers.length > 0) {
			...
			$('#table-mapping tbody').html(json.columns.map(item => `<tr>
				<td>${item}</td>
				<td>
					<select class="form-control field-header select2 headers" data-toggle="select2" data-field="${item}" name="mapping[${item}]">
						${headers}
					</select>
				</td>
			</tr>`).join());
