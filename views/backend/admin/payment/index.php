<?php
$this->db->select('SUM(IF(payment.currency_code != "INR", payment.amount * currency.exchange_rate, payment.amount)) as amount', false);
$this->db->where('payment.date_added >=' , date('Y-m-d', strtotime('+1 day', strtotime(date('Y-m-d', strtotime('-1 year'))))));
$this->db->where('payment.date_added <=' , date('Y-m-d', strtotime(date('Y-m-d', strtotime('+1 day')))));
$this->db->where('payment._deleted', 0);
$this->db->order_by('payment.date_added' , 'DESC');
$this->db->join('currency', 'currency.code = payment.currency_code');
$last_year_revenue = $this->db->get('payment')->row()->amount;

$this->db->select('SUM(IF(payment.currency_code != "INR", payment.amount * currency.exchange_rate, payment.amount)) as amount', false);
$this->db->where('payment.date_added >=' , date('Y-m-d', strtotime('+1 day', strtotime(date('Y-m-d', strtotime('-1 month'))))));
$this->db->where('payment.date_added <=' , date('Y-m-d', strtotime(date('Y-m-d', strtotime('+1 day')))));
$this->db->where('payment._deleted', 0);
$this->db->order_by('payment.date_added' , 'DESC');
$this->db->join('currency', 'currency.code = payment.currency_code');
...
$this->db->select('SUM(IF(payment.currency_code != "INR", payment.amount * currency.exchange_rate, payment.amount)) as amount', false);
$this->db->where('payment.date_added >' , date('Y-m-d', strtotime('+1 day', strtotime(date('Y-m-d', strtotime('-1 week'))))));
$this->db->where('payment.date_added <=' , date('Y-m-d', strtotime(date('Y-m-d', strtotime('+1 day')))));
$this->db->where('payment._deleted', 0);
$this->db->order_by('payment.date_added' , 'DESC');
$this->db->join('currency', 'currency.code = payment.currency_code');
...
$this->db->select('SUM(IF(payment.currency_code != "INR", payment.amount * currency.exchange_rate, payment.amount)) as amount', false);
$this->db->where('payment.date_added >' , date('Y-m-d', strtotime('-0 day')));
$this->db->where('payment.date_added <=' , date('Y-m-d', strtotime('+1 day')));
$this->db->where('payment._deleted', 0);
$this->db->order_by('payment.date_added' , 'DESC');
$this->db->join('currency', 'currency.code = payment.currency_code');
...
$this->db->select('SUM(IF(payment.currency_code != "INR", payment.amount * currency.exchange_rate, payment.amount)) as amount', false);
$this->db->where('payment.date_added >' , date('Y-m-d', $timestamp_start));
$this->db->where('payment.date_added <=' , date('Y-m-d', $timestamp_end));
$this->db->where('payment._deleted', 0);
$this->db->order_by('payment.date_added' , 'DESC');
$this->db->join('currency', 'currency.code = payment.currency_code');
$total = $this->db->get('payment')->row()->amount;
		...
		<div class="card">
			<div class="card-body">
				...
				<div class="row justify-content-md-center">
					<div class="col-xl-6">
						<form class="form-inline" action="<?php echo site_url('admin/payment/filter_by_date_range') ?>" method="get">
							<div class="col-xl-10">
