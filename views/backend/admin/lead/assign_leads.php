...
<script>
	$(document).ready(function() {
		table = $('#ajax-datatable').DataTable({
			"ajax": "<?php echo site_url('admin/ajax_assign_leads/' . $archived); ?>",
			"processing": true,
			"serverSide": true,
			"order": [
				[1, "desc"]
...
$(function() {
	$(document).on('change', '#filter_telecaller_id, #filter_site_id, #filter_event_id, #filter_location', function() {
		table.ajax.url("<?= base_url('admin/ajax_assign_leads/' . $archived); ?>?site_id=" + $('#filter_site_id').val() + '&telecaller_id=' + $('#filter_telecaller_id').val() + '&event_id=' + $('#filter_event_id').val() + '&location=' + $('#filter_location').val()).load();
	})
});
</script>
