...
<div class="row">
	<div class="col-xl-12">
		<div class="card">
			<div class="card-body">
				<h4 class="mb-3 header-title"><?php _el('leads'); ?></h4>

				<div class="buttons text-right">
					<?php if (in_array($this->session->userdata('role_id'),[1])) { ?>
						<a href="<?=base_url("admin/download_lead/lead")?>" class="btn btn-primary">Download Leads</a>
					<?php } ?>
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-body p-3">
								...
								<form action="<?php echo site_url('admin/status/add'); ?>" method="post" id="form-status">
									<input type="hidden" name="lead_id" value="" />
									<div class="form-group">
										<label for="class"><?php _el('feedback'); ?></label>
										<select
				...
				<div class="modal fade" id="telecaller-modal">
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-body p-3">
								<form action="<?php echo site_url('admin/assign_telecaller'); ?>" method="post" id="form-telecaller">
									<div class="form-group">
...
const archive = () => {
	if (confirm('<?php echo _li('Are you sure?'); ?>')) {
		...
		submitForm('<?php echo site_url('admin/bulk_archive'); ?>', fd , json => {
			if (json.success) {
				success_notify(json.success);
				window.location.reload()
			} else {
...
const archived = (id) => {
	if (confirm('<?php _eli('Are u sure?'); ?>')) {
		...
		submitForm('<?php echo site_url('admin/lead_archived'); ?>', fd, json => {
			if (json.success) {
				success_notify(json.success);
...
const getLeadStatus = (id) => {
	...
	submitForm('<?php echo site_url('admin/lead_status'); ?>', fd, json => {
		let html = '';
...
const showDetails = (id) => {
	...
	submitForm('<?php echo site_url('admin/lead_detail'); ?>', fd, json => {
		...
		const html = `<div class="modal fade" id="edit-modal">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body p-3">
						<form action="<?php echo site_url('admin/edit_lead'); ?>" method="post" id="form-lead-edit">
...
$(document).ready(function() {
	table = $('#ajax-datatable').DataTable({
		"ajax": "<?php echo site_url('admin/ajax_lead/' . $archived); ?>",
		"processing": true,
		"serverSide": true,
		"order": [[ 1, "desc" ]],
		"aoColumnDefs": [
...
<script>
$(function() {
	$(document).on('change', '#filter_telecaller_id, #filter_site_id, #filter_book_status, #filter_lead_status, #filter_page_count, #filter_event_id, #filter_location', function() {
		table.ajax.url("<?= base_url('admin/ajax_lead/' . $archived); ?>?site_id=" + $('#filter_site_id').val() + '&book_status=' + $('#filter_book_status').val() + '&verified=' + $('#filter_lead_status').val() + '&page_count=' + $('#filter_page_count').val() + '&telecaller_id=' + $('#filter_telecaller_id').val() + '&event_id=' + $('#filter_event_id').val() + '&location=' + $('#filter_location').val()).load();
	})
});
</script>
