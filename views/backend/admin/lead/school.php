	...
	<div class="col-xl-12">
		<div class="card">
			<div class="card-body">
				...
				<!-- <div>
					<select class="form-control select2" required data-toggle="select2" name="telecaller_id" id="telecaller" onchange="window.location='?telecaller_id=' + this.value">
		...
		<div class="card">
			<div class="card-body">
				...
				<div class="buttons text-right">
					<button class="btn btn-success" id="approved_lead"><?=_l('approve_lead')?></button>

					<a href="<?= base_url('admin/download_lead/school') ?>" class="btn btn-info"><?= _l('download_leads') ?></a>

					<button class="btn btn-danger" onclick="archive()">
						<?php _el($archived ? 'restore' : 'archive'); ?>
					</button>
					<button class="btn btn-info" data-toggle="modal" data-target="#telecaller-modal">
						<?php _el('assign_telecaller'); ?>
					</button>
				</div>
				...
				<div class="modal-content">
					...
					<div class="modal-body p-3">
						<form action="<?php echo site_url('admin/edit_school_lead'); ?>" method="post" id="form-lead-edit">
							...
							<div class="text-left pt-2" id="show_error" display-none> 
							</div>
							<div class="text-right pt-2">
								<button
									type="submit"
...
<?php
// if (!empty($this->input->get('telecaller_id'))) {
// 	$telecaller_id = 'telecaller_id='.$this->input->get('telecaller_id');
// }else{
// 	$telecaller_id = '';
// }
?>
<script>
	$(document).ready(function() {
		table = $('#ajax-datatable').DataTable({
			"ajax": "<?php echo site_url('admin/ajax_school_lead/' . $archived); ?>" + window.location.search,
			"processing": true,
			"serverSide": true,
			"order": [
				[1, "desc"]
					...
					render: function(data, type) {
						if (type === 'display') {
							...
							<ul class="dropdown-menu">
								...
								<li>
									<a class="dropdown-item"
									href="/admin/approved_lead/${data.id}"
									><?php _el('verify'); ?></a>
								</li>
								<li>
									<a class="dropdown-item"
									href="/admin/reject_lead/${data.id}"
									><?php _el('reject'); ?></a>
								</li>
							</ul>
						</div>`
...
<script>
	$(document).ready(function() {
		$('#ajax-datatable').on('xhr.dt', function(e, data) {
			console.log({
				data
			})
			$('#approved_lead').text(`<?php echo _li('${data.json.approved_lead} approved '); ?>`)
		});
	});
</script>
...
$(function() {
	$(document).on('change', '#filter_telecaller_id, #filter_site_id, #filter_event_id, #filter_location', function() {
		table.ajax.url("<?= base_url('admin/ajax_school_lead/' . $archived); ?>?site_id=" + $('#filter_site_id').val() + '&telecaller_id=' + $('#filter_telecaller_id').val() + '&event_id=' + $('#filter_event_id').val() + '&location=' + $('#filter_location').val()).load();
	})
});
</script>
