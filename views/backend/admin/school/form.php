			...
			<div class="card-body">
			  <div class="col-lg-12">
				...
				<form class="required-form" action="<?php echo $action ; ?>" method="post" enctype="multipart/form-data">
					...
					<div class="form-group">
						<label><?php _el('site'); ?></label>
						<select class="form-control select2" data-site="<?=$details['site_id'] ?? ''?>" data-toggle="select2" name="site_id" id="site_id">
					...
					<div class="form-group">
						...
						<select class="form-control select2" data-toggle="select2" name="student_id[]" id="enrol_students"  multiple="multiple">
							<?php if (0) { ?>
							<?php foreach ($students as $student) { ?>
							<?php if (in_array($student['id'], ($details['student_id'] ?? []))) { ?>
							<option value="<?php echo $student['id']; ?>" selected><?php echo $student['user'] . ' ' . $student['course']; ?></option>
							<?php } else { ?>
							<option value="<?php echo $student['id']; ?>"><?php echo $student['user'] . ' ' . $student['course']; ?></option>
							<?php } ?>
							<?php } ?>
							<?php } ?>
						</select>
						<a href="javascript:void(0)" onclick="$('#enrol_students option').prop('selected', true);$('#enrol_students').trigger('change');"><?php _el('select_all'); ?></a> /
						<a href="javascript:void(0)" onclick="$('#enrol_students option').prop('selected', false);$('#enrol_students').trigger('change');"><?php _el('unselect_all'); ?></a>
					</div>
...
<script>
$(function() {
	$('#course, #site_id').on('change', function() {
		let fd = new FormData();
		fd.append('course_id', $('#course').val());
		fd.append('site_id', $('#site_id').val());
		...
		renderSelect('enrol_students', '<?php _el('select_student'); ?>', [<?php echo implode(',', $details['student_id'] ?? []); ?>])
	});
