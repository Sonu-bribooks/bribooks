...
final class Dtdcb2b {
	...
	public function __construct() {
		if (ENVIRONMENT === 'production') {
			$this->api_url = 'https://pxapi.dtdc.in/api/';
		} else {
			$this->api_url = 'https://alphademodashboardapi.shipsy.io/api/';
		}
	...
	public function bookOrder($data = []) {
		...
		$payload = ['consignments' => [[
			...
			'origin_details'	=> [
				...
				'pincode'		   	=> 122017, // $data['pickup_location']['pincode'],
	...
	public function generateLabel($shipment_id = false) {
		...
		$response = self::_curl(
			'customer/integration/consignment/shippinglabel/stream?reference_number=' . $shipment_info['awb_number'] . '&label_code=SHIP_LABEL_4X6&label_format=pdf',
			$payload,
			'GET',
			$this->api_key
		);
