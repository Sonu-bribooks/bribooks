...
final class SchoolRanking_lib {
	public function __construct() {
		...
		$this->load->model('ranking/school/SchoolRankingCountry_model');
		$this->load->model('ranking/school/SchoolRankingState_model');
		$this->load->model('ranking/school/SchoolRankingCity_model');
		...
		$this->ranking_city_model 		= $this->CI->SchoolRankingCity_model;
		$this->ranking_state_model 		= $this->CI->SchoolRankingState_model;
		$this->ranking_country_model 	= $this->CI->SchoolRankingCountry_model;
	...
	use StateSchoolRanking, CountrySchoolRanking, CitySchoolRanking, BuildSchoolRanking;

	public function updateCountryRankById($rank_id = 0, $score = 0, $type = 'country') {
		self::_pushUpdate($rank_id, $score, $type);
	}
	...
	private function _getRegisteredCount($event_id = 0, $school_id = 0) {
		if (in_array($school_id, [1, 2])) return 0;
	...
	private function _getKey($rank_info = [], $type = 'city') {
		return vsprintf('live_school_%s_ranks_%s_%s_%s_%s', [
			$type,
	...
	private function _getAlertUserKey($rank_info = [], $user_id = 0, $type = 'city') {
		return vsprintf('%s_%s_school_rank_update_%s_%s_%s', [
			$type,
	...
	private function _getAlertLiveUserKey($rank_info = [], $type = 'city') {
		return vsprintf('event_live_%s_school_%s_%s_%s', [
			$type,
			(int)$rank_info['event_id'],
			(int)$rank_info[sprintf('event_challenge_%s_id', $type)],
			(int)$rank_info[sprintf('%s_id', $type)],
