...
final class Icode_lib {
	private $_endpoint		= NULL;
	private $_ch			= NULL;
	private $_response		= NULL;
	private $_res_headers	= NULL;
	private $_header		= [];
	private $_option		= [];
	private $_set			= [];
	private $_where			= [];
	...
	public function __construct() {
		...
		$this->_endpoint = $this->CI->config->item('api_icode');
	...
	public function get_data($table, $set = NULL, $escape = NULL) {
		$this->_table = $table;
	...
	private function _query_get() {
		...
		$this->_get($this->_table, $this->_set + $this->_where + $this->_like + $additional);
	...
	private function _post($endpoint = NULL, $data = []) {
		...
		$this->curl($endpoint, $this->_json ? json_encode($data) : http_build_query($data));
	...
	private function curl($endpoint, $data = NULL) {
		...
		$this->_res_headers = http_parse_headers($explode[0]);

		if (($this->_res_headers['Response Code'] ?? '') == 100) {
			$this->_res_headers	= http_parse_headers($explode[1]);
			$this->_response	= $explode[2];
		} else {
			$this->_response	= $explode[1] ?? '';
		}
